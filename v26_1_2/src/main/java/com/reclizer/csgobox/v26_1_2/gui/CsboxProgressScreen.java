...
public class CsboxProgressScreen extends Screen {
     ...
     * in renderBg throws "Can only blur once per frame"). With the Blur mod
     * loaded, defer to vanilla so the mod's mixins keep driving the radius.
     */
    @Override
    ...
    private void renderBg(GuiGraphicsExtractor guiGraphics, float partialTicks) {
        ...
        // Bright golden marker line, like the original. Height matches the
        // card as shown at the line: the lens centre is the golden line
        // itself, so the stopped card is the magnified strip card
    ...
    public void tick() {
        ...
        if (serverWinningIndex == null) {
            ...
            int winningIndex = Mth.clamp(this.serverWinningIndex, 0, Math.max(0, this.itemInput.size() - 1));
            // The winning strip entry carried full data components (the decoy
            // entries travel component-free), so it is the granted item itself.
            this.resultItem = this.itemInput.get(winningIndex).copy();
            return;
        }
