...
public final class BoxJsonLoader {
     ...
     * Returns the produced diagnostics; a result with {@code ok == false} must
     * not be loaded. Diagnostics are rolled back so a dry run never pollutes
     * the live error list.
     */
    ...
    private static Optional<BoxDefinition> parseFromBytes(byte[] bytes, Path file, PriceTable priceTable) {
        ...
        // uppercase, CJK, #...) would either crash ResourceLocation.parse or
        // produce a silently different id.
        ...
        try {
            // v2.1.0 gating: "enabled":false skips cleanly; "requires" skips
            // with a clear error when any mod is absent.
