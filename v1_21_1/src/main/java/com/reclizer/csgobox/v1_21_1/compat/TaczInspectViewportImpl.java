...
public final class TaczInspectViewportImpl {
     ...
     * inspect via {@link IClientPlayerGunOperator#inspect()}. Caller should
     * close the look screen immediately so the player sees the native
     * first-person inspect animation.
     */
    ...
    /**
     * Whether TACZ's inspect animation is currently playing for this gun. Used
     * by the client-tick session to know when the first-person inspect has
     * finished so the UI can be restored. Matching on any running animation
     * whose name contains "inspect" (gun-pack inspect states vary: inspect /
     * inspect_empty / custom names).
     */
    ...
    /** End the first-person inspect session: stop sounds, exit the state
     *  machine and restore the player's previous main hand. */
