...
public class TerminalBootScreen extends Screen {
    ...
    // Button colour tokens (match the 26.x ButtonPalette OPEN/CLOSE styles).
    private static final int OPEN_FILL = 0xFF1F6B33;
    private static final int OPEN_FILL_HOVER = 0xFF2A8042;
    private static final int OPEN_BORDER = 0xFF2EA348;
    private static final int OPEN_BORDER_HOVER = 0xFF45C26A;
    private static final int OPEN_TEXT = 0xFFE8F5E9;
    private static final int CLOSE_FILL = 0xFF3A4148;
    private static final int CLOSE_FILL_HOVER = 0xFF4A535C;
    private static final int CLOSE_BORDER = 0xFF6C7680;
    private static final int CLOSE_BORDER_HOVER = 0xFF8B96A0;
    private static final int CLOSE_TEXT = 0xFFE6EAEE;
    ...
    public void render(GuiGraphics gg, int mouseX, int mouseY, float partialTick) {
        ...
        this.itemDrag.tick();
        renderBg(gg, partialTick, mouseX, mouseY);
    ...
    private void renderGrid(GuiGraphics gg, Player player) {
        if (player == null) {
            return;
        }
        GuiRegion.Region listArea = GuiRegion.list(this.width, this.height);
    ...
    protected void renderLabels(GuiGraphics gg, int mouseX, int mouseY) {
        ...
        int openColor = isInside(mouseX, mouseY, openButtonX(), buttonY(),
                actionButtonWidth(), buttonHeight()) ? 0xFFFFFFFF : OPEN_TEXT;
        renderCenteredText(gg, Component.translatable("gui.csgobox.terminal.open").withStyle(style).getVisualOrderText(),
                openButtonX(), buttonY(), actionButtonWidth(), buttonHeight(), 0.8F, openColor);
        int closeColor = isInside(mouseX, mouseY, closeButtonX(), buttonY(),
                actionButtonWidth(), buttonHeight()) ? 0xFFFFFFFF : CLOSE_TEXT;
        renderCenteredText(gg, Component.translatable("gui.csgobox.terminal.cancel").withStyle(style).getVisualOrderText(),
                closeButtonX(), buttonY(), actionButtonWidth(), buttonHeight(), 0.8F, closeColor);
    }

    private void drawOpenButton(GuiGraphics gg, int mouseX, int mouseY) {
        boolean hover = isInside(mouseX, mouseY, openButtonX(), buttonY(),
                actionButtonWidth(), buttonHeight());
        drawButton(gg, openButtonX(), buttonY(), actionButtonWidth(), buttonHeight(),
                OPEN_FILL, OPEN_BORDER, OPEN_FILL_HOVER, OPEN_BORDER_HOVER, hover);
    }

    private void drawCloseButton(GuiGraphics gg, int mouseX, int mouseY) {
        boolean hover = isInside(mouseX, mouseY, closeButtonX(), buttonY(),
                actionButtonWidth(), buttonHeight());
        drawButton(gg, closeButtonX(), buttonY(), actionButtonWidth(), buttonHeight(),
                CLOSE_FILL, CLOSE_BORDER, CLOSE_FILL_HOVER, CLOSE_BORDER_HOVER, hover);
    }
    ...
    private void drawButton(GuiGraphics gg, int x, int y, int w, int h,
        ...
        AnimRenderOps.fill(gg, x, y, x + w, y + h, border);
        AnimRenderOps.fill(gg, x + 1, y + 1, x + w - 1, y + h - 1, fill);
    }
    ...
    private void renderText(GuiGraphics gg, FormattedCharSequence text, float px, float py, float scale) {
        RenderFontTool.drawString(gg, this.font, text, px, py, 0, 0, scale, 0xFFD3D3D3);
    }

    private void renderCenteredText(GuiGraphics gg, FormattedCharSequence text,
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            if (isInside(mouseX, mouseY, openButtonX(), buttonY(),
                    actionButtonWidth(), buttonHeight())) {
                Minecraft.getInstance().setScreen(new TerminalScreen(this.terminalStack));
                return true;
            }
            if (isInside(mouseX, mouseY, closeButtonX(), buttonY(),
                    actionButtonWidth(), buttonHeight())) {
                onClose();
                return true;
            }
