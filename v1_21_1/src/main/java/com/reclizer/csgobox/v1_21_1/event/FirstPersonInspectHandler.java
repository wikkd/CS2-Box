...
public final class FirstPersonInspectHandler {
    ...
    /**
     * Begin a first-person inspect session from the box look screen.
     * Captures the player's current main hand for later restore, injects the
     * gun and starts TACZ's native draw (the inspect input fires once the draw
     * releases the state lock). Returns false if it could not start (caller
     * keeps the GUI).
     */
