...
public class CsLookItemScreen extends Screen {
    ...
    private static final ResourceLocation ICON_INSPECT =
            new ResourceLocation("csgobox:textures/gui/toolbar/inspect.png");
    private static final ResourceLocation ICON_GLOVES =
            new ResourceLocation("csgobox:textures/gui/toolbar/gloves.png");
    private static final ResourceLocation ICON_MODEL =
            new ResourceLocation("csgobox:textures/gui/toolbar/model.png");
    private static final ResourceLocation ICON_INFO =
            new ResourceLocation("csgobox:textures/gui/toolbar/info.png");
    private static final ResourceLocation ICON_STICKER =
            new ResourceLocation("csgobox:textures/gui/toolbar/sticker.png");
    private static final ResourceLocation ICON_MORE =
            new ResourceLocation("csgobox:textures/gui/toolbar/more.png");
    ...
    private void renderBg(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (!this.taczDisplayChecked && this.minecraft != null && this.minecraft.player != null) {
            ...
            if (TaczInspectViewport.isAvailable(this.openItem)) {
                if (TaczInspectViewport.enterDisplay(this.openItem, this.minecraft.player)) {
        ...
        if (this.taczViewportActive && this.minecraft != null && this.minecraft.player != null) {
            viewportRendered = TaczInspectViewport.renderViewport(guiGraphics, openItem,
                    ...
                    previewPixelX() + (int) (8 * scale), previewPixelY() + (int) (8 * scale),
                    scale);
            if (!viewportRendered) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int btnW = backButtonWidth();
        int btnH = this.height * 5 / 100;
        if (button == 0 && ButtonPalette.isInside(mouseX, mouseY, btnX, btnY, btnW, btnH)) {
            this.onClose();
            return true;
        }
        ...
        if (button == 0 && ButtonPalette.isInside(mouseX, mouseY, inspectX, inspectY, infoSize, infoSize)) {
            boolean available = this.minecraft != null && this.minecraft.player != null
                    && TaczInspectViewport.isAvailable(this.openItem);
            if (available) {
                if (this.taczViewportActive) {
                    // Viewport already showing the 3D model: play the inspect animation.
        ...
        if (button == 0 && ButtonPalette.isInside(mouseX, mouseY, glovesX, glovesY, infoSize, infoSize)) {
            boolean available = this.minecraft != null && this.minecraft.player != null
                    && TaczInspectViewport.isAvailable(this.openItem);
            if (available) {
                if (this.taczViewportActive) {
                    // Toggle off: leave the viewport, fall back to the plain 3D item render.
                    TaczInspectViewport.exit(this.openItem);
                    this.taczViewportActive = false;
                } else if (TaczInspectViewport.enterDisplay(this.openItem, this.minecraft.player)) {
                    this.taczViewportActive = true;
                }
            }
            return true;
        ...
        if (button == 0 && ButtonPalette.isInside(mouseX, mouseY, infoX, infoY, infoSize, infoSize)) {
