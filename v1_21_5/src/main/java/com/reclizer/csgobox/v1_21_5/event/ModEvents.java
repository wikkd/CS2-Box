...
import com.reclizer.csgobox.v1_21_5.box.BoxDefinition;
...
public final class ModEvents {
    ...
    public static void playerLoggedIn(net.neoforged.neoforge.event.entity.player.PlayerEvent.PlayerLoggedInEvent event) {
        if (CsgoBox.CONFIG.enableAchievements()
            ...
            com.reclizer.csgobox.v1_21_5.advancement.ModLoadedTrigger.INSTANCE.trigger(sp);
        }
    }
    /**
     * Periodically prunes expired open-cooldown entries from
     * {@link com.reclizer.csgobox.v1_21_5.packet.PacketCsgoProgress#tickOpenBlockMap(long)} so the map stays bounded.
     */
    @SubscribeEvent
    public static void serverTick(ServerTickEvent.Pre event) {
        if (event.getServer().getTickCount() % 100 == 0) {
            com.reclizer.csgobox.v1_21_5.packet.PacketCsgoProgress.tickOpenBlockMap(event.getServer().overworld().getGameTime());
        }
    }
