...
- `common/box/BoxOdds.java` — 纯概率数学（`totalWeight` / `gradeChance` / `itemChance` / `percent` / v2.1.0 起 `weightedItemChance` / `positiveItemWeightSum`）；JEI 分类、`/csbox info`、箱子 tooltip 概率行共用同一来源（与服务端 roll 同口径）
- `item/ItemCsgoBox.java` — 箱子物品；`appendHoverText` 显示 5 档概率行 + 每档物品清单（v2.1.0 加权时逐物品显示概率）；`buildGradeMap()` 构建加权池；`applyIcon` 应用每箱图标（26.x 支持 ITEM_MODEL，1.21.1/1.20.1 仅 CMD）
- `box/BoxItemCodec.java` — v2.1.0 物品解析（`weight` / `count:[min,max]` / `#tag` 展开 / `loot_table` / `enchant` / 缺模组 id 区分；`item_spec` 标记）；forge_1_20_1 用 NBT 变体、v1_21_1 保留 TACZ 特判
- `box/BoxItemResolver.java`（v2.1.0 新增，每平台一份）— 开箱/购买时解析 `item_spec`（count 区间、随机附魔、loot_table 掷表），仅服务端调用
