...
- **不生成任何箱子默认配置（含终端机）**：2.0.0 起终端机也出厂即**空箱**——`terminal.json` 不再自动生成，终端机与默认箱行为一致（打开为空谈判屏，不绑定奖池）。旧版（无 `type` 且带遗留 `key`）的 `terminal.json` 会自动迁移；空文件视为合法未配置状态；损坏文件备份为 `terminal.json.corrupt-<时间戳>` 后移出加载路径（不回写默认配置）。
