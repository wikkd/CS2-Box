...
| `random` | 浮点数组[5] | 否 | 5 个等级权重(grade1 到 grade5 顺序)。**v2.0.1 起全部为 0/负 = 该箱不可开启**（服务端拒开且不消耗钥匙/箱子，`/csbox validate` 会提示） |
...
| `enabled` | 布尔 | 否 | v2.0.1：`false` 整箱跳过（不注册、不报错，作者主动停用）。默认 `true` |
| `requires` | 字符串数组 | 否 | v2.0.1：所需模组 id 列表。任一未加载时整箱跳过并报「缺少依赖」错误（联动包防半残箱子） |
| `icon` | 字符串/整数 | 否 | v2.0.1：每箱图标。整数 = CustomModelData（配资源包）；`ns:path` 字符串 = item-model id（26.x 直接渲染；1.21.1 / 1.20.1 记警告跳过） |
| `discount` | 浮点数 | 否 | v2.0.1：终端售价折扣（0..1，如 `0.2` = 8 折）。仅终端机有意义 |
| `stock` | 整数 | 否 | v2.0.1：终端**全局**库存上限（-1 = 无限，默认）。售罄后终端显示空态并拒绝购买。内存态，重启恢复满库存 |
| `max_per_player` | 整数 | 否 | v2.0.1：每人开箱上限（-1 = 无限，默认）。内存态，重启清零；单次/批量均生效，验证通过后才消耗钥匙 |
| `cooldown_seconds` | 整数 | 否 | v2.0.1：每箱开箱冷却（0 = 无，默认） |
| `permission` | 字符串 | 否 | v2.0.1：权限节点。默认放行；整合包通过 `CsgoBox.PERMISSION_GATE` 钩子接入权限后端（如 LuckPerms） |
| `pity` | 对象 | 否 | v2.0.1：保底策略 `{"grade": "classified", "every": 20}`——连续 `every-1` 次未开出目标档（或更高）后，下一次强制从目标档及以上按权重抽取。内存态按「玩家×箱子」计数，重启清零；单开/批量均生效，只有实际发放才推进计数。`grade` 必须是 5 档 id 之一、`every ≥ 2`，否则静默不生效（校验器会提示）。开箱预览屏会显示「保底：还差 N 次」（v2.0.2 起） |
| `format_version` | 整数 | 否 | v2.0.2：配置格式版本标记，可选（缺省视为旧版）；新文件建议写 `2`。运行时校验类型（正整数） |

> **文件名校验（v2.0.1）**：文件名（不含 `.json`）即箱子 id，只允许小写字母、数字、
> `_`、`.`、`/`、`-`。非法文件名（空格、大写、中文、`#` 等）直接拒绝加载并提示合法示例。

> **JSON Schema（v2.0.1）**：`docs/box-schema/box.schema.json` 提供完整字段定义
> （含全部新字段），VS Code / IDEA 配置后即可离线补全与校验。运行时仍以 Java 校验器为准。
> **未知字段警告（v2.0.2 起）**：运行时校验器会点名顶层与物品级的未知键（拼写错误、
> 已废弃字段），仅提示不阻断加载——出现该警告时优先检查键名拼写。
...
- `count`:数量(默认 1)；v2.0.1 支持 `[min,max]` 区间（开箱/购买时随机，预览显示下限）
- `weight`(可选, v2.0.1):档内权重(默认 1,越大越常见;`0` 临时禁用该条目,不用删配置)
- `tag`(可选, v2.0.1):**以 `#` 开头**的物品标签引用,如 `"#minecraft:swords"`——加载时展开为当前成员物品,自动跟随整合包增减。不带 `#` 的 `tag` 仍是旧版 NBT 字符串(向后兼容)
- `loot_table`(可选, v2.0.1):战利品表引用,如 `"minecraft:chests/simple_dungeon"`——开箱时服务端掷该表(预览显示桶占位)。可联动任意模组战利品表
- `enchant`(可选, v2.0.1):随机附魔快捷——`true`(任意可附魔随机)或 `{"id": "minecraft:sharpness", "level": [3,5]}`(指定附魔与等级/区间)
- `price` **已移除(v2.0.1 起)**:终端价格不再写在箱子 JSON 里,统一由 `config/csbox/_prices.json` 管理(见 [3.3 价格表](#33-价格表v210-起))。残留的 `price` 字段会作为 schema 错误上报(`/csbox info error`),**但在下次 `/csbox reload`(或热重载/重启)时会自动迁移进价格表并清零该字段**(见「旧版自动迁移」),迁移前该物品没有价格（终端机不售卖、拆解为 0，直到补价）。购买时实际成交价 = 价格表中的价(范围价 = 当次报价随机采样价)+ 磨损惩罚(仅对无耐久条物品,按磨损百分比加价:`ceil(基础价 × 20% × 磨损值)`,满磨损 +20%,向上取整),再叠加箱子 `discount` 折扣
- `components`(可选):Minecraft 1.21+ data components
- 旧版 `tag` 字符串字段**仍可加载**(向后兼容)

> **v2.0.1 完整示例**：见 `compat-packs/apotheosis-pack/gem_crate.json`（`requires`、
...
- **范围价随机(v2.0.1)**:值写成 `[min, max]`(闭区间,两元素均为非负整数且
...
- **未定价即报错(v2.0.1,无默认价回退)**:箱子物品加载时按物品 id(或 `id#变体`)
...
- **旧版自动迁移(v2.0.1 适配)**:每次 `loadAll` / `/csbox reload`(含热重载)前,
...
- **拆解台计价对齐(v2.0.1)**：武库拆解台对**盖章物品**先按价格表计价——该物品 id
...
### 3.4 村民动态定价（`_villager_prices.json`）

军火商职业村民的交易价格锚定 **`config/csbox/_prices.json`**（见 3.3），并由
**`config/csbox/_villager_prices.json`** 统一调参。首次启动各平台自动生成默认文件
（`BoxDefaults.writeVillagerPricesIfMissing`，在 `commonSetup` 触发）。

**字段与默认值**

| 字段 | 默认 | 含义 |
|---|---|---|
| `enabled` | `true` | 总开关；`false` 时全部交易回退静态值（见下） |
| `buy_rate` | `0.06` | 矿物→点数收购倍率（表价 × buy_rate） |
| `sell_markup` | `1.0` | key1 / key2 出售加成（合成成本 × sell_markup） |
| `fluctuation` | `0.10` | 每次刷新重采样幅度（±10%） |
| `key0_price` | `9` | key0 货币锚（9 点 = 1 key0） |
| `key0_fluctuation` | `0.05` | key0 单独波动（±5%） |
| `box_key_ratio` | `0.9` | box 售价 = key0 × 0.9 |
| `terminal_key_ratio` | `2.0` | terminal 售价 = key0 × 2.0 |
| `fallbacks` | iron 50 / gold 50 / diamond 200 / emerald 55 | 价格表缺矿物条目时的兜底表价 |

**定价规则**（每次村民交易刷新在 `±fluctuation` 内重采样，表价是区间时先采样区间再乘）

- 收购：1 铁 / 绿宝石 / 金 / 钻 → `表价 × buy_rate × (1 ± fluctuation)` 点。
- key0：`key0_price × (1 ± key0_fluctuation)` 点。
- key1 / key2：按合成成本锚定 `3 × 矿物表价 × sell_markup × (1 ± fluctuation)`；key2 因
  点数堆叠上限 64 拆成「≤64 点 + 整钻」两段（钻数为 0 时按 1 计，保证双输入报价不消失）。
- box / terminal：`key0 × box_key_ratio / terminal_key_ratio × (1 ± fluctuation)`。
- 防套利：默认 `buy_rate = 0.06` 下 3 铁 ≈ 9 点 = key0 成本，矿 → 点 → key0 无利可图。

**平台实现差异**

- **1.20.1 / 1.21.1**：`VillagerTrades` 代码注册，每次交易刷新调用 common
  `VillagerPricing.quote` 现场报价。
- **26.x（v26_1_2 / v26_2 / forge_26_1_2 / forge_26_2）**：交易仍由 datapack
  `trade_set` + `villager_trade` JSON 定义（`common/src/main/resources/data/csgobox/villager_trade/arms_dealer/**`），
  价格字段引用自定义 loot `NumberProvider` `csgobox:arms_dealer_price`（注册于
  `loot_number_provider_type`，平台类 `ArmsDealerPriceProvider`）：
  - 收购端（矿→点）：`gives.count` 固定，用 `minecraft:set_count` loot function 动态改写点数；
  - 出售端（点→key / box / terminal）：`wants.count` / `additional_wants.count` 直接引用 provider；
  - 每个 JSON 条目携带 `quote_field`（`iron` / `emerald` / `gold` / `diamond` / `key0` /
    `key1` / `key2_points` / `key2_diamonds` / `box` / `terminal`）与 `fallback`（= 原静态值）。

**行为**

- 波动频率：村民生成 / 升级 / 数据包重载时重采样（与旧平台 `updateTrades` 时机一致）；
  `restock` 只重置次数不重定价（原版机制）。
- `enabled=false` 或配置缺失：26.x 回退 JSON 内 `fallback`（2 / 2 / 4 / 12 / 8 / 9 / 24 /
  18 / 45+1 钻），1.20.1 / 1.21.1 回退等价静态值——平台间数值对齐。
- 配置为懒加载缓存：改动 `_villager_prices.json` 后**重启生效**（26.x 与旧平台一致，未做热重载）。

### 3.5 默认文件生成

首次启动时 `BoxJsonLoader.loadAll()` 会保证 `config/csbox/` 目录存在，并：

- **不生成任何箱子默认配置（含终端机）**：2.0.0 起终端机也出厂即**空箱**——`terminal.json` 不再自动生成，终端机与默认箱行为一致（打开为空谈判屏，不绑定奖池）。旧版（无 `type` 且带遗留 `key`）的 `terminal.json` 会自动迁移；空文件视为合法未配置状态；损坏文件**保留原位**（让 `/csbox info error` 持续报告）并另存 `terminal.json.corrupt-<时间戳>` 备份副本（v2.0.1 行为，此前是移出加载路径）。
...
- 现有 JSON 文件**不会被覆盖**(只读)
- **`/csbox validate [box]`（v2.0.1）**：干跑校验——解析指定箱子（或全部）**不注册、不进缓存**，
  只报告「能否加载」与诊断；作者可循环「改→验→改」而不影响线上状态。校验结果不污染
  `/csbox info error` 列表。
- **加载诊断（v2.0.1 增强）**：`/csbox info error` 现在还会报告「空档位」（某档 0 个有效物品，
...
## 7. 箱子与物品注册表（联机注意，v2.0.1）

**箱子是数据，不是物品 id。** 2.0.1 起物品注册表是**编译期常量**，不再随 `config/csbox/` 变化：
...
- **联机**：客户端与服务端的 `config/csbox/` **不再需要保持一致**（多客户端以服务端下发为准）。
  这正是 2.0.1 修复的事故：以前每个箱子各占一个物品 id，两侧配置不同（或 `requires` 门控的
