...
| `stock` | 整数 | 否 | v2.0.1：终端**全局**库存上限（-1 = 无限，默认）。售罄后终端显示空态并拒绝购买；`restock_minutes` 补货。内存态，重启恢复满库存 |
| `restock_minutes` | 整数 | 否 | v2.0.1：补货间隔（分钟）。`0` = 永不自动补货（默认） |
...
| `pity` | 对象 | 否 | v2.0.1：保底策略 `{"grade": "classified", "every": 20}`——连续 `every-1` 次未开出目标档（或更高）后，下一次强制从目标档及以上按权重抽取。内存态按「玩家×箱子」计数，重启清零；单开/批量均生效，只有实际发放才推进计数。`grade` 必须是 5 档 id 之一、`every ≥ 2`，否则静默不生效（校验器会提示） |
