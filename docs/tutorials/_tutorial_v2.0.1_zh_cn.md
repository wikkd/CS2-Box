...
> 本文件是模组内置教程的在线版：教程随 mod 打包（离线可用），首次启动复制到 `config/csbox/` 下、文件名带版本号。英文版见 [`_tutorial_v2.0.0.md`](./_tutorial_v2.0.0.md)。
...
- 终端机没有 `key` 字段，不消耗钥匙。
- 出价用武库点数算。成交价取物品 JSON 里的 `price`（设了就按它收），没设就回退到档位默认价（grade1 = 6、grade2 = 10、grade3 = 16、grade4 = 22、grade5 = 30）。物品没有耐久条的话还要加磨损惩罚，按磨损**百分比**加价：加价 = 基础价 × 20% × 磨损值（向上取整），满磨损（战痕累累）最多 +20%。
