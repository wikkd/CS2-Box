...
> Auto-generated reference for the box JSON files in `config/csbox/`. The companion Simplified Chinese version is copied alongside it (file name stamped with the current mod version, e.g. `_tutorial_v2.0.0_zh_cn.md`).
...
> As of this release the item object has **no `price` field** — terminal
> prices come exclusively from `config/csbox/_prices.json`, see
...
Terminal item prices in Armory Points are centrally managed in
**`config/csbox/_prices.json`** — never inside box files. The `_` prefix keeps
it out of the box scan. Format is a flat map of **item id → non-negative integer**:
...
- The same item id costs the same in every terminal (no per-box override).
  Items without an entry have **no price**: the terminal never offers them,
  the recycler pays 0, and a box containing an unpriced id item refuses to
  load (add the price to `_prices.json` first).
- NBT-distinguished items (e.g. TACZ guns/ammo) use an `id#variant` sub-key:
  the loader reads `GunId` / `AmmoId` from the entry's legacy `tag` string.
  A variant lookup prefers the variant key, then falls back to the plain id.
- The mod never creates an empty table, but legacy (v2.0.x) `price` fields
  still sitting in box JSONs are **auto-migrated** into this table on the next
  `/csbox reload` (or hot reload / restart), and the `price` field is stripped
  from the box file (idempotent). When the same item has different prices in
  different boxes, the migration uses the **average (rounded half up)**;
  prices already in the table win and are never overwritten. `#tag` /
  `loot_table` entries and invalid prices (negative / fractional) cannot be
  migrated and stay in place, still erroring.
- `/csbox validate` checks this table, and `/csbox reload` (or hot reload)
...
Use `minecraft:air` as the `key` field for a box that requires no key.
...
> loot pool. `csgobox:terminal` itself is statically registered (same as
> `csgobox:csgo_box`), so it always exists even without a `terminal.json` —
> it then opens as an **empty crate** (no loot bound); create `terminal.json`
> yourself to give it a loot pool.

- Terminals have no `key` field, so opening one never consumes a key.
- Offers are priced in Armory Points. The price comes from
  `config/csbox/_prices.json` (looked up by item id, see
  [Price table](#price-table)); items without an entry fall back to the
  default grade-level price (grade1 = 6, grade2 = 10, grade3 = 16,
  grade4 = 22, grade5 = 30). Items without a durability bar take a wear
  penalty instead: a percentage markup of the item price — surcharge =
  price × 20% × wear value, rounded up, so a Battle-Scarred (full-wear)
  item costs at most 20% more.
...
- A leftover item-level `price` field is reported as a load error, but the
  next `/csbox reload` **auto-migrates** it into `_prices.json` and strips the
  field (conflicting prices are averaged).
- In `_prices.json`, keys must be `ns:path` (optionally `#variant`) and
  values must be non-negative integers; invalid entries are reported and
  skipped.
...
Entity drop rates are taken from the per-entity rate if present, else the default `drop` field.
