...
> Auto-generated reference for the box JSON files in `config/csbox/`. The companion Simplified Chinese version is at [`_tutorial_v2.0.0_zh_cn.md`](./_tutorial_v2.0.0_zh_cn.md).
...
- Offers are priced in Armory Points. The price is the per-item `price`
  ...
  grade5 = 30). Items without a durability bar take a wear penalty instead:
  a percentage markup of the item price — surcharge = price × 20% × wear
  value, rounded up, so a Battle-Scarred (full-wear) item costs at most
  20% more.
