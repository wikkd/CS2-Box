...
`forge_26_1_2` 是只读实验模块（MinecraftForge 26.1.2，源码未提交、不在 CI、不参与镜像纪律），由 `scripts/port-forge-2612.py` 从 `v26_1_2` 机械转换 + 手工适配。

- 审查点：是否误把 `forge_26_1_2` 当正式平台发布、或要求它进入 CI 矩阵 / 镜像纪律？该模块为本地 WIP，审查时**忽略其编译状态**。
