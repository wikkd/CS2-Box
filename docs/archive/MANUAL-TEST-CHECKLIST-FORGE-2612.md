> ⚠️ 已归档（历史快照）：本文档记录 forge_26_1_2 1.0.6 手动测试清单的当时状态，不再随项目更新；当前信息以 README.md 与 docs/ 为准。
...
- [ ] F9 教程刷新：首次启动 `config/csbox/` 生成教程 md；`/csbox reload tutorial` 可从包内刷新（dev 运行版本号 `unknown` 属已知限制，正式 jar 安装不受影响）
