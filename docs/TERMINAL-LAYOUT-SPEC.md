...
>
> **2026-08-11 审美回归注记**：HTML 原型迁移后经 AI 视觉 + 人工视觉两轮测试（用户逐区域圈图迭代），
> 排版参数已按实际游戏渲染效果重新定标（字号 0.47~0.72 紧凑系，§1 差距表全部清零），
> 并以用户要求为准：不再以 HTML 像素为唯一基准。下表各「现状/差距」列为历史对照记录，
> 最终效果以人工视觉回归（docs/RUNTIME-UI-TESTING.md）与 §6 TL 断言为准。
