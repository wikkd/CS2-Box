...
- **新增 `box-editor/` 可视化网页配置工具**（初始版）：纯前端零依赖、`file://`
  ...
  **固定价或 `min-max` 范围价**（输入 `1500` 或 `1500-3000`，导出为 `[1500, 3000]`，
  ...
  状态共享 + 跨标签同步）；内置「TACZ 导入教程」弹窗（中英双语，8 步含
  枪械/弹药变体、原始 NBT、id#变体 价格、26.x 回退、reload 说明）；批量粘贴
  可直接识别游戏内 `/csbox nbt hand` 聊天输出（标题/复制按钮行自动忽略，
  截断输出提示改用聊天复制按钮取完整 JSON）；原始 NBT tag 与 Data Components
  收进物品卡底部「高级数据」折叠区（默认收起避免普通玩家误读，有内容时自动
  展开并标注"已填写"）；页面背景为随仓库分发的封面图（`img/bg.jpg`，固定
  铺满），整体界面改为**奶白色毛玻璃**风格（半透明奶白组件 + `backdrop-filter`
  模糊，透出背景封面图，浅色文字/控件配色全套适配）；修复
...
2.0.0 是本模组自 1.0.6 以来最大的一次功能跃迁：从「单纯的开箱动画」扩展为一条**完整的 CS 风格军备经济线**——

- **开箱核心**：恢复并加固批量开箱（服务端权威 + 异步预计算 + 流水揭晓），钥匙判定、创造模式消耗、扣减时机等边界全部收口；
- **终端机（本版本最大新增玩法）**：一台可谈判的「军火商终端」——用武库点数向军火商买货，5 轮报价逐轮讨价还价，服务端权威会话（重开续谈 / 转手锁死 / 超时·谈崩·成交三终态销毁物品 / 世界级持久化）；
- **世界生成与经济闭环**：军火商职业村民 + 武库商小屋（野外据点 + 原版村庄自然生成双通道），开箱 → 武库点数 → 拆解/终端购买 → 再开箱的循环成型；
- **可查、可扩展**：JEI 开箱概率查询（多人同步）+ KubeJS/Java 事件三件套，整条经济线对整合包作者脚本化开放；
- **平台与工程**：平台数增至 6（新增 MC 1.20.1 Forge），全部 26.x Forge 实验模块追平功能线；渲染统一收口到每平台唯一的 `AnimRenderOps` 门面（CI 漂移门禁守护），Blur 模组软适配 + `blurRadius` 背景模糊增强。

### ⭐ 重点更新

| 重点 | 内容 | 覆盖平台 |
|---|---|---|
| ⭐ **终端机谈判系统** | 武库点数购买 + 5 轮谈判 + 随机磨损报价（无耐久物品按磨损加罚价）+ 3 小时倒计时；服务端权威会话锁（重开续谈）、世界级持久化、转手锁死；成交 / 谈崩（5 轮全拒）/ 超时三种终态均销毁终端机物品 | NeoForge 4 平台 + forge_26_1_2 |
| ⭐ **武库商与军火商生态** | 军火商职业村民（防解雇/防消失加固）、武库商小屋野外据点（structure_set + 商店宝箱）、**接入原版 5 群系村庄道路自然生成**；手建版结构模板替换脚本版 | 全平台（纯数据驱动） |
| ⭐ **批量开箱恢复** | 1.0.6 屏蔽解除：Shift+右键总览屏 → 二次确认 → 服务端权威计数 + 异步线程池预计算 + 主线程扣减 + 底部上升流水揭晓屏；异步窗口越界缺陷已修 | NeoForge 3 平台 + forge_26_1_2 |
| ⭐ **JEI 开箱概率 + 多人同步** | 自定义「开箱概率」分类（服务端同口径概率）、任意掉落物反查来源箱、`PacketSyncBoxDefinitions` 全量同步盒定义（MP 客户端 JEI 完整可用） | v1_21_1 / v26_1_2 / v26_2 |
| ⭐ **KubeJS/Java 扩展三事件** | `BoxOpeningEvent`（可取消）/ `TerminalBuyEvent` / `ArmoryRecycleEvent`（可取消）——开箱前置校验、终端经济记账、拆解黑名单一站式脚本化，`docs/KUBEJS-EVENTS.md` 附示例 | NeoForge 3 + forge_26_1_2 |
| ⭐ **新平台：MC 1.20.1（Forge）** | 第 6 个平台模块 `forge_1_20_1`（MinecraftForge 47.4.22 / Java 17），全功能线回移；Networking/Capability/渲染三大重写，TACZ 检视完整保留 | forge_1_20_1 |
| ⭐ **TACZ 第一人称检视** | TACZ 枪械新增独立第一人称检视屏（真实枪械模型视口 + 检视动画），无 TACZ 环境静默降级 | v1_21_1 / forge_1_20_1 |
...
> 终端机是一条独立的「谈判式购买」玩法线：以武库点数向军火商购买开箱产物，5 轮报价可接受可拒绝；会话由服务端全权持有——重开续谈、转手锁死、持久化续算，成交 / 谈崩 / 超时三种终态都会销毁终端机物品本身。
...
- **恢复批量开箱**（1.0.6 屏蔽 → 2.0.0 恢复）：移除 1.0.6 的两处屏蔽——客户端入口 `ClickEvent` 的 shift 硬编码 `false`（恢复 `mc.options.keyShift.isDown()`，Shift+右键进总览屏）与 `PacketCsgoBulkProgress` 的 `BULK_OPEN_ENABLED` 服务端开关；恢复服务端权威计数（箱/钥匙/`bulkOpenCount` 上限）→ 异步线程池计算 → 主线程扣减的完整链路；另修复恢复后暴露的边界缺陷：`finalizeBulkOpen` 复核时 `actualK` 未 clamp 到已计算结果数（异步窗口内库存增长会致 `results.subList` 越界、整批中止且重试必失败），现按 `Math.min(actualK, results.size())` 截断，剩余箱子下轮再开；1.21.1 / 26.1.2 / 26.2 三平台同步恢复（forge_26_1_2 实验模块一并同步）。

#### JEI 集成与多人同步

- **JEI 开箱概率分类 + 盒定义多人同步**：新增 JEI 集成（`v1_21_1` / `v26_1_2` / `v26_2` 三平台）——自定义「开箱概率」分类（recipeType `csgobox:box_open`），每个箱子一条配方：钥匙输入槽、箱子输出槽、物品池输出网格 20 格（4×5，超出画「…共 N 件」省略），左侧文本列显示掉落率、各级权重百分比（按 `grade.color()` 着色，最多 4 行 + 「…共 N 级」省略）与实体掉率行（实体名经 `EntityChineseMap`）；任意掉落物可反查「哪个箱子掉、单件概率多少」（tooltip「[等级] 权重 x/y · 单件 z%」）；分类图标用箱子物品栈（回退 `csgo_box`）。钥匙 `smithing_transform` 为原版数据驱动配方，JEI 已自动显示、无需代码适配。概率口径 = 服务端实际算法（common 新增纯函数 `BoxOdds`：`totalWeight` / `gradeChance` / `itemChance`，与 `OddsCalculator.pickGrade` / `GradeMap.pickRandom` 语义一致，`:common:test` 新增 7 用例全绿）。**盒定义多人同步**：新增 `PacketSyncBoxDefinitions`（playToClient 全量 `List<BoxDefinition>`，复用 `BoxDefinition.STREAM_CODEC`），玩家加入 / `/csbox reload` / 文件热重载后广播，客户端 `clear()`→`register()` 重灌注册表并触发 JEI 配方刷新（`BoxJeiSync` 静态桥，无 JEI 时零依赖）——纯多人客户端 JEI 数据完整可用，顺带修复此前 MP 客户端缺盒定义只能回退默认权重的问题。JEI 版本固定（19.44.0.401 / 29.22.0.73 / 30.20.0.154），`neoforge.mods.toml` 可选依赖（`mandatory=false`，side CLIENT）。`forge_26_1_2` 跳过：Modrinth 无 JEI 26.x Forge 构建。三平台 `clean compileJava` 通过。
...
- **forge_1_20_1 实验模块（MinecraftForge 47.4.22 / MC 1.20.1 回移）**：新增第 6 个平台模块，将 2.0.0 线全部功能移植到 MinecraftForge 47.x（ForgeGradle 7.x + Java 17）：以 `forge_26_1_2` 为基准，复用 `common/` 全部纯 Java 逻辑；三大重写区域——Networking 改 `SimpleChannel`（14 个 packet）、Capability 走 `LazyOptional` + `AttachCapabilitiesEvent`、渲染层 `GuiGraphics` 直调（无 PIP 系统，`AnimRenderOps.renderItem3D` 降级 2D、`supports3D()` 返回 false）；DataComponent 存储回退为 ItemStack NBT，`StreamCodec`/`RegistryFriendlyByteBuf` 改 `FriendlyByteBuf` 手动序列化。不入 CI 矩阵与 3 平台镜像纪律/AnimRenderOps 漂移门禁，不入正式发行矩阵；JEI 集成暂未实现（与其他 forge 模块现状一致）。

- **forge_26_1_2 纳入同步开发（2.0.0 线）**：随 `v26_1_2` 基准同步终端机 / 军火商 / 村民等文件，手工适配 forge API（`ModItems` 补 `armory_point` / `terminal` 注册与创造栏条目、`ModBlocks` POI `Set` 包装、`CsboxConfig` 补 `BackgroundStyle` UI 段、`AnimRenderOps.renderItem3D` 适配 forge `Icon3DRenderState` 13 参 Euler 签名、`IconListTools` 移植 alpha 淡入版本、`ButtonPalette` 补 `CLOSE`、`GuiItemMove` 补 `Quat` 重载、`PacketTerminalBuy` 改 forge `CustomPayloadEvent` API、`Networking` 注册 terminal 包）；`build.gradle` 删除 2.0.0 排除清单；`clean compileJava` 通过，`test-forge-2612.sh` L0-L3 门禁 7/7 PASS，`PlatformSmokeTest` 基线守卫更新为 2.0.0 同步断言。

- **forge_26_2 追平 2.0.0 线**：以 `forge_26_1_2` 为基准整模块迁移（`scripts/port-forge-262.py` 包名 + Forge 26.1.2→26.2 API 映射 + 手工适配：`Options.hideGui` → `HudVisibility`、`setScreen` → `setScreenAndShow`、advancement 包迁移、PIP 渲染器保持 Forge 欧拉角方案），`build.gradle` 排除清单删除、`PlatformSmokeTest` 断言 2.0.0 物品；2026-08-18 全面审计：5 平台 `clean compileJava` 全通过、`test-forge-262.sh` 7/7 PASS、版本四同步 OK、资源一致性补齐（4 个物品定义从 `forge_26_1_2` 补入）。
...
- **ForgeGradle 固定 7.0.31（forge_1_20_1 构建加固）**：`forge_1_20_1/build.gradle` 插件声明由动态范围 `[7.0.17,8)` 固定为 `7.0.31`（与 `gradle.properties` 的 `forgegradle_version` 一致）——离线 / maven.minecraftforge.net 受限环境下，范围声明会解析到最新插件标记（如 7.0.35）而其实现构件未缓存、构建直接失败；同时明确 TACZ 不进 userdev 运行环境（生产 jar 的 mixin refmap 与 dev 官方映射名不匹配会崩），运行时验证走独立 `prod/` 目录装载正式 jar。
...
- **TACZ 第一人称检视（v1_21_1 / forge_1_20_1，可选集成）**：新增 `gui/FirstPersonInspectScreen` + `event/FirstPersonInspectHandler` 与 `compat/TaczInspectViewport`（接口 + Impl 双文件）——抽出 TACZ 枪械时可进入独立的第一人称检视屏，渲染 TACZ 真实枪械模型视口并播放检视动画；`CsLookItemScreen` 工具栏与 `CsgoBox` 注册接线，`ModList.isLoaded("tacz")` 门禁，未装 TACZ 或物品非枪械时静默降级为普通检视路径（不影响其它平台）。
...
- **forge_26_1_2（MinecraftForge 26.1.2-64.1.0，Java 25）随 1.0.6 发行纳入 git 管理**。发行时先保持 1.0.6 特性基线（`forge_26_1_2/build.gradle` 对 common 源集/资源排除 1.0.7 线增量），**同日转入 1.0.7 同步开发线**：`build.gradle` 排除清单已删除，随 `v26_1_2` 基准同步终端机 / 军火商 / 村民 / Blur / 批量开箱恢复等增量（见上文 [2.0.0-beta] 节），`clean compileJava` 与 `test-forge-2612.sh` L0-L3 门禁 7/7 PASS；仍为实验模块，不入三平台正式发行矩阵与 CI。
...
## [2.0.0] - 历史计划快照（原 1.0.7 计划）

> 以下计划已全部实现，实现记录见上方 [2.0.0-beta]；本节保留为历史计划快照。
