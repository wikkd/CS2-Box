...
### 未完成(等 stage 6)
- v26_2 GUI / PIP 运行时回归(参见上一节列出的破坏性变更)。编译已经通过,但实际 3D 旋转在 26.2 decoupled 渲染引擎下需要用户手玩验证。
- HUD-overlay 回归:MC 26.2 移除了 `Options.hideGui` 字段,开箱时 hotbar/血条仍然显示。需要后续在 26.2 找到新的 HUD 隐藏机制(目前没有等价 API)。
