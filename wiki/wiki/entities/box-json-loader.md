...
- `LoadError` 收集：`LAST_LOAD_ERRORS` 静态列表，`loadFromFile()` 把所有异常分支收集为 `LoadError`（含 file/boxId/reason/line/column/cause）；`getLastLoadErrors()`/`hasLoadErrors()` 供 `/csbox errors` 和 `LoadErrorAnnouncer` 消费
