...
- **用例重点（box_variants）**：合法变体（不同 key/`drop` 概率/`random` 权重/entity 列表/grade 缺失/单条目箱）+ **错误箱子自检**（非法 JSON/缺 name/缺 key/权重非法/id 不存在/重复/grade 非数组）→ 模组拒绝加载、`/csbox errors` 能查、不崩溃、其他箱不受影响。
- **排除**：批量开箱（当前版本屏蔽）、CI 接入（10 平台 2-4 小时，本地执行）。
