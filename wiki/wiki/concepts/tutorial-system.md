...
- **版本管理**：`BoxDefaults.writeTutorialIfMissing` + `refreshTutorials`（`/csbox reload tutorial` 强制重下）；版本不匹配时 `deleteStaleTutorials` 按 `^_tutorial_v.*\.md$` 白名单**直接删除**（1.0.8 取代 OS 回收站 + `.trash/` 两级回收机制，无回收站）
