...
public final class ModItems {
    ...
    // component plus the server-synced BoxRegistry. Only the ids that ship with
    // the mod keep a dedicated item (so /give csgobox:gun_crate keeps working
    // and old stacks stay valid); custom boxes use the generic csgo_box /
    // terminal item and are handed out with /csbox give.
     ...
     *  own box id in the {@code csgobox:box_id} data component. The 26.x
     *  ITEM_MODEL component is repointed at the shared csgo_box / terminal
     *  model, exactly like the old dynamic registrations did. */
        ...
        return ITEMS.register(id, () -> terminal
                ? new ItemTerminal(itemProperties(id)) {
                    ...
                    public ItemStack getDefaultInstance() {
                        ...
                        stack.set(DataComponents.ITEM_MODEL, Identifier.parse(CsgoBox.MODID + ":terminal"));
                        return stack;
                    }
                }
                : new ItemCsgoBox(itemProperties(id)) {
                    ...
                    public ItemStack getDefaultInstance() {
                        ItemStack stack = super.getDefaultInstance();
                        ItemCsgoBox.setBoxId(boxId, stack);
                        stack.set(DataComponents.ITEM_MODEL, Identifier.parse(CsgoBox.MODID + ":csgo_box"));
                        return stack;
                    }
                });
    }
     ...
     * {@code csgobox:box_id} data component).
