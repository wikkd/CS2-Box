...
> 本文件随 mod 内置（离线可用），首次启动时由模组复制到 `config/csbox/` 下，文件名带当前版本号（如 `_tutorial_v2.0.1.md`）。英文版见同目录英文文件。
...
> v2.0.1 起**物品对象里不再有 `price` 字段**——终端机价格的唯一来源是
...
v2.0.1 起，所有终端机的物品成交价（武库点数）统一放在 **`config/csbox/_prices.json`**
