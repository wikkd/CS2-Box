...
public final class IconListTools {
     ...
     * line is scaled up (left-edge anchored) with a gold border, soft glow and
     * a brightened interior so the passing strip "pops" through the selection
     * slot. {@code focusScale} is 1.0 at rest and grows toward
     * {@link #FOCUS_PEAK_SCALE} as the card approaches the line.
     */
    public static void renderItemProgressFocus(LivingEntity entity, GuiGraphicsExtractor guiGraphics,
                                               ItemStack itemStack, float pX, float pY, float width, float height,
                                               int grade, float focusScale) {
        int color = ColorTools.colorItems(grade);
