...
public final class ModItems {
     ...
     * box id in the {@code csgobox:box_id} data component. These ids have no
     * model file of their own on 26.x, so the shared csgo_box / terminal model
     * is stamped as well (the 1.20.1 port remaps it at bake time instead).
     */
    private static Supplier<Item> fixedBoxItem(String id, boolean terminal) {
        ...
        Identifier model = Identifier.fromNamespaceAndPath(CsgoBox.MODID, terminal ? "terminal" : "csgo_box");
        return ITEMS.registerItem(id, props -> terminal
                ? new ItemTerminal(props) {
                    @Override
