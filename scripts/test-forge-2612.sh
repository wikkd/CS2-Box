#!/usr/bin/env bash
# test-forge-2612.sh — forge_26_1_2 (MinecraftForge) 模块一键回归（测试流程 L0-L3）。
#
...
#   S5  :forge_26_1_2:test             —— PlatformSmokeTest（入口可加载 + 1.0.6 基线守卫）
#
# 用法:
#   scripts/test-forge-2612.sh                 # 全量
#   scripts/test-forge-2612.sh --skip-test     # 跳过 S5（例如只验证构建门禁）
#   scripts/test-forge-2612.sh --skip-drift --skip-version
#   FORGE_TEST_TIMEOUT=3600 scripts/test-forge-2612.sh   # 覆盖 Gradle 阶段超时（默认 1800s）
#
...
stage() {
  ...
  if [ "${rc}" -eq 0 ]; then
    pass "${desc}"
    CASES_XML+="    <testcase classname=\"forge-2612.${name}\" name=\"${name}\" time=\"0\"/>"$'\n'
  else
    fail "${desc} (exit ${rc})"
    CASES_XML+="    <testcase classname=\"forge-2612.${name}\" name=\"${name}\" time=\"0\"><failure message=\"exit ${rc}\"/></testcase>"$'\n'
  fi
}
...
mc="$(grep -E '^mc_version_forge_26_1_2=' gradle.properties | head -1 | cut -d= -f2- | tr -d '[:space:]')"
if [ -z "${mod_version}" ] || [ -z "${mod_id}" ] || [ -z "${mc}" ]; then
  echo "ERROR: cannot read mod_version/mod_id/mc_version_forge_26_1_2 from gradle.properties" >&2
  exit 2
fi
...
echo "============================================================"
echo " forge_26_1_2 回归  |  mod_version=${mod_version}  |  artifact=${ARTIFACT}"
echo "============================================================"
echo
...
if [ "${SKIP_COMPILE}" -eq 0 ]; then
  ...
  CASES_XML+="    <testcase classname=\"forge-2612.S1\" name=\"S1 clean+compile\" time=\"0\"><skipped/></testcase>"$'\n'
fi
...
if [ "${SKIP_JAR}" -eq 0 ]; then
  ...
  if [ "${rc}" -ne 0 ]; then
    ...
    if [ -f "${ARTIFACT}" ] && [ -s "${ARTIFACT}" ]; then
      ...
      CASES_XML+="    <testcase classname=\"forge-2612.S2\" name=\"jar-artifact\" time=\"0\"/>"$'\n'
    else
      fail "产物缺失或为空 (${ARTIFACT})"
      CASES_XML+="    <testcase classname=\"forge-2612.S2\" name=\"jar-artifact\" time=\"0\"><failure message=\"missing\"/></testcase>"$'\n'
    fi
    if unzip -p "${ARTIFACT}" META-INF/mods.toml 2>/dev/null | grep -q "version=\"${mod_version}\""; then
      pass "mods.toml 版本 = ${mod_version}"
      CASES_XML+="    <testcase classname=\"forge-2612.S2\" name=\"mods-toml-version\" time=\"0\"/>"$'\n'
    else
      fail "mods.toml 未展开为 version=\"${mod_version}\""
      CASES_XML+="    <testcase classname=\"forge-2612.S2\" name=\"mods-toml-version\" time=\"0\"><failure message=\"version mismatch\"/></testcase>"$'\n'
    fi
  fi
else
  warn "S2 skipped (--skip-jar)"
  CASES_XML+="    <testcase classname=\"forge-2612.S2\" name=\"jar\" time=\"0\"><skipped/></testcase>"$'\n'
fi
...
if [ "${SKIP_VERSION}" -eq 0 ]; then
  ...
  CASES_XML+="    <testcase classname=\"forge-2612.S3\" name=\"version-sync\" time=\"0\"><skipped/></testcase>"$'\n'
fi
...
if [ "${SKIP_DRIFT}" -eq 0 ]; then
  ...
  CASES_XML+="    <testcase classname=\"forge-2612.S4\" name=\"animops-drift\" time=\"0\"><skipped/></testcase>"$'\n'
fi

# ---- S5 PlatformSmokeTest ----
if [ "${SKIP_TEST}" -eq 0 ]; then
  log "S5 :forge_26_1_2:test (timeout ${FORGE_TEST_TIMEOUT}s)..."
  ...
  CASES_XML+="    <testcase classname=\"forge-2612.S5\" name=\"platform-smoke-test\" time=\"0\"><skipped/></testcase>"$'\n'
fi
...
mkdir -p "${REPORT_DIR}"
{
  ...
  echo "<testsuite name=\"forge-2612\" tests=\"$((PASS+FAIL))\" failures=\"${FAIL}\" skipped=\"${WARN}\">"
  printf '%s' "${CASES_XML}"
  echo "</testsuite>"
} > "${REPORT_DIR}/forge-2612.xml"

echo
echo "============================================================"
echo " 结果: PASS=${PASS}  FAIL=${FAIL}  WARN=${WARN}"
echo " 报告: ${REPORT_DIR}/forge-2612.xml"
for r in "${RESULTS[@]}"; do
  echo "  ${r}"
done
echo "============================================================"
