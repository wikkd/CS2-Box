...
`common/src/main/resources/` 由两个平台通过 `srcDir project(':common').file('src/main/resources')` 共享(v26_1_2 额外设置 `duplicatesStrategy = EXCLUDE`)。
...
# 3. 启动开发客户端
./gradlew :v26_1_2:runClient   # 或 :v1_21_1:runClient
```
...
- ⏳ Phase 7+ 未开始:common 完整业务代码迁移、容器化布局(P1-1)、per-item 视觉基线(P1-3)、三档设计 token(P2-2)
