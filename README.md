...
1. 下载最新版本的 `csgobox-x.x.x.jar` 文件（从构建产物或 Release 页面获取）。
2. 将 JAR 文件放入游戏安装目录的 `mods` 文件夹中。
3. 确保同时安装了 **NeoForge 21.1.115+** for Minecraft 1.21.1。
4. 启动游戏，加载存档即可。

> 注意：本模组需要同时在客户端和服务端安装才能正常工作。

## 快速开始

1. 进入游戏后，打开创造模式物品栏，在 `CS Box` 分组中找到 `CS 箱子`。
2. 右键点击箱子物品打开箱子界面。
3. 查看各等级（等级 1 到 5）的奖励预览。
4. 使用对应的钥匙（铁钥匙/金钥匙/钻石钥匙/下界合金钥匙）开启箱子。
5. 观看开箱动画，获得随机奖励物品。
...
在游戏中使用管理员命令获取物品（需要权限等级 2）：

```bash
/csbox give csgobox:csgo_box 1 @p
/csbox give csgobox:csgo_key0 3 @p
```

### 创建自定义箱子

1. 在 `config/csbox/` 目录下创建新的 JSON 文件。
2. 使用 `/csbox add <箱子名称>` 创建箱子。
3. 使用 `/csbox add <箱子> <等级> hand <数量>` 添加手中物品到指定等级。
4. 使用 `/csbox reload` 重新加载配置。

### 查看箱子详情

```bash
/csbox list                    # 列出所有已注册的箱子
/csbox info <箱子名称>          # 查看箱子的详细配置
```

### 管理箱子

```bash
/csbox give <箱子> [数量] [玩家]   # 给予玩家箱子物品
/csbox reload                   # 重新加载所有箱子配置
/csbox set <箱子> <等级> weight <权重>  # 修改等级权重
```

## 命令权限

所有 `/csbox` 命令需要玩家具有 **权限等级 2**（原版默认的 OP 级别）。
