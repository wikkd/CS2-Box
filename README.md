...
> 全部平台共享同一 `mod_version`，jar 命名规则为 NeoForge `csgobox-<mc>-<mod_version>.jar`、Forge `csgobox-forge-<mc>-<mod_version>.jar`（**唯一例外：Forge 1.20.1 发布物为 `csgobox-forge-1.20.1-<mod_version>-srg.jar`**）。已归档（EOL）的 1.21.0/3/4/5/8/10/11 旧版玩家可继续使用既有发布产物。
...
- ✅ 2.0.0（[2.0.0] 发行）：批量开箱恢复 + UI 打磨（无二次确认、可滚动「显示全部」网格）、终端机谈判会话（随机磨损 + 无耐久物品磨损点数惩罚）、武库商小屋世界生成结构、JEI 开箱概率分类、`blurRadius` 背景模糊等；6 平台（NeoForge 3 + Forge 3）同步正式发布
