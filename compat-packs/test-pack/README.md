...
| `test_terminal.json` | 终端机 | `type: terminal`、`discount` 折扣、`stock` 库存 + `restock_minutes` 补货、`loot_table` 条目（开箱掉落，不参与终端报价）、范围价 |
...
4. **终端机**：`test_terminal` 售罄后（stock 50）显示空态，`restock_minutes: 30` 后补货；`discount: 0.2` 所有成交 8 折。
