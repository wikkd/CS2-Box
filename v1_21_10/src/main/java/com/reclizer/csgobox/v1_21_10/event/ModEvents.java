package com.reclizer.csgobox.v1_21_10.event;
...
public final class ModEvents {
    ...
    public static void playerLoggedIn(net.neoforged.neoforge.event.entity.player.PlayerEvent.PlayerLoggedInEvent event) {
        if (CsgoBox.CONFIG.enableAchievements()
            ...
            com.reclizer.csgobox.v1_21_10.advancement.ModLoadedTrigger.INSTANCE.trigger(sp);
        }
    }
    /**
     * Periodically prunes expired open-cooldown entries from
     * {@link com.reclizer.csgobox.v1_21_10.packet.PacketCsgoProgress#tickOpenBlockMap(long)} so the map stays bounded.
     */
    @SubscribeEvent
    public static void serverTick(ServerTickEvent.Pre event) {
        if (event.getServer().getTickCount() % 100 == 0) {
            com.reclizer.csgobox.v1_21_10.packet.PacketCsgoProgress.tickOpenBlockMap(event.getServer().overworld().getGameTime());
        }
    }
